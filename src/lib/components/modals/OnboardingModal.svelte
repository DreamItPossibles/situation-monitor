			...
			<div class="preset-grid">
				{#each PRESET_ORDER as presetId}
					...
					<button class="preset-card" onclick={() => handleSelectPreset(presetId)}>
						...
						<div class="preset-name">{$t(`presets.${presetId}.name`)}</div>
						<div class="preset-description">{$t(`presets.${presetId}.description`)}</div>
						<div class="preset-panel-count">
							{$t('onboarding.panels_count', { values: { count: preset.panels.length } })}
						</div>
					</button>
				{/each}
			</div>
